...
then
  ...
  tests=(resources/tests/GanacheTests/AssignLocalAdd.json)
fi
...
do
  echo "---------------------------------------------------------------"
  echo "running Obsidian Ganache Test $test"
  echo "---------------------------------------------------------------"
  ...
  ACCT2=$(echo "$ACCTS" | jq '.result[1]' | tr -d '"')
  echo "ACCT is $ACCT"
  ...
  # Obtain a gas estimate without adding the transaction to the blockchain
  PARAMS=$( jq -ncM \
                 --arg "fn" "$ACCT" \
...
do
  echo "---------------------------------------------------------------"
  echo "running Solidity Ganache Test $test"
  echo "---------------------------------------------------------------"
  ...
  # get the binary representation of the test
  # grep through the format output by solc in yul producing mode for the binary representation
  
  EVM_BIN=$(docker run -v "$( pwd -P )":/sources ethereum/solc:stable --bin /sources/resources/tests/SolidityTests/"$NAME".sol)
  EVM_BIN="${EVM_BIN//[!0-9]/}"
  echo "$EVM_BIN"
  
  # compile the contract to evm, also creating the directory to work in, failing otherwise
  if ! docker run -v "$( pwd -P )":/sources ethereum/solc:stable -o /sources/"$NAME" --optimize --asm --overwrite /sources/resources/tests/SolidityTests/"$NAME".sol > "$NAME".evm
  then
      echo "$NAME test failed: cannot compile sol to evm"
      failed+=("$test [compile sol to evm]")
      exit 1
  fi
  ...
  PARAMS=$( jq -ncM \
               ...
               --arg "vn" "0x0" \
               --arg "dn" "0x$EVM_BIN" \
               '{"from":$fn,"to":$tn,"gas":$gn,"gasPrice":$gpn,"value":$vn,"data":$dn}')
