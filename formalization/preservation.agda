...
preservation ty@(locTy {Γ} {Δ₀} {T₁ = contractType t₁} {T₂ = contractType t₂} {T₃ = contractType t₃} l spl@(unownedSplit namesEq1 namesEq2 permsEq permUnowned))
                       consis@(ok Δ voidLookup boolLookup objLookup refConsistencyFunc)
                       st@(SElookup {Σ} {l = l} {v = objRef o} _ lookupL) = 
  let
    spl' = unownedSplit refl (Eq.trans (Eq.sym namesEq1) namesEq2) refl permUnowned
