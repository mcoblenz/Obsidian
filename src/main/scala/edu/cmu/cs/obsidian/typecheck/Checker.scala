...
case class Context(underlyingVariableMap: Map[String, ObsidianType], isThrown: Boolean, transitionFieldsInitialized: Set[(String, String, AST)]) {
    ...
    def lookupFieldTypeInType(typ: ObsidianType) (fieldName: String): Option[ObsidianType] = {
       // XXX TODO! val typTable = contractTable.lookupContract()
        val foundField = doLookup(fieldName, (field: String) => typ.table.lookupField(field), typ)
         ...
         e match {
             ...
             case Dereference(eDeref: Expression, fieldName) =>
                 ...
                 val fieldType =  context.lookupFieldTypeInType(derefType)(fieldName) match {
                     case Some(typ) => typ
                     case None =>
                         logError(e, FieldUndefinedError(derefType.extractSimpleType.get, fieldName))
                         return (BottomType(), contextPrime)
                 }
