...
class CodeGen (val target: Target) {
    ...
    def translateConstructors(decls: Seq[Constructor],
        ...
        val argList: Seq[(String, JVar)] = mainArgs.map((arg: VariableDeclWithSpec) =>
            (arg.varName, newDispatcher.param(resolveType(arg.typIn), arg.varName))
        )
        ...
        def genArgCopy(argPair: (VariableDeclWithSpec, String)): Unit = argPair match {
            case (arg, argName) => newDispatcher.body().decl(resolveType(arg.typIn), arg.varName, JExpr.ref(argName))
        }
