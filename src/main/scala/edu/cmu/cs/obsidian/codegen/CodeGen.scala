...
class CodeGen (val target: Target) {
    ...
    private def translateStubDeclaration(decl: Declaration,
          ...
          decl match {
            ...
            case f@Field(_, _) => translateStubField(f, inClass)
            ...
            case t@Transaction(_, _, _, _, _) => if (!txNames.contains(t.name)) translateStubTransaction(t, inClass, stOption)
                                                 txNames.add(t.name)
            case s@State(_, _) => translateStubState(s, inClass, txNames)
