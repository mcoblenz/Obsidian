        ...
        for (c <- program.contracts) {
            ...
            c match {
                case obsCon: ObsidianContractImpl => {
                    val newClass = if (obsCon.isInterface) {
                        programPackage._interface(obsCon.name)
                    } else {
                        programPackage._class(obsCon.name)
                    }
    ...
    private def unmarshallExprExpectingFullObjects(translationContext: TranslationContext,
        ...
        initialTyp match {
            ...
            case np: NonPrimitiveType =>
                ...
                val classInstance = constructNew(translationContext, initialTyp, translationContext.contract)

                val tryBlock = cond._else()._try()

                tryBlock.body().assign(unarchivedObjDecl, classInstance)
                ...
                tryBlock._finally().assign(unarchivedObjDecl, JExpr._null())
...
//        }
        ...
        if (!aContract.isInterface) {
            ...
            if (aContract.params.nonEmpty) {
                val strListType = model.ref("java.util.ArrayList").narrow(model.ref("java.lang.String"))
                newClass.field(JMod.PRIVATE, strListType, genericParamsFieldName)
            }
    ...
    */
    ...
    def constructNew(translationContext: TranslationContext, typ: ObsidianType, contract: Contract,
                     constructorParams: IJExpression*): JInvocation =
        typ match {
            case np: NonPrimitiveType =>
                ...
                if (idx >= 0) {
                    ...
                    val classExpr = classForNameMeth
                            .arg(JExpr.ref(genericParamsFieldName).invoke("get").arg(JExpr.lit(idx)))

                    val getConstructorInvoke = JExpr.cast(model.ref("java.lang.Class").narrow(typeVar), classExpr)
                            .invoke("getConstructor")

                    withArgs(getConstructorInvoke, constructorParams).invoke("newInstance")
                } else {
                    JExpr._new(resolveType(typ, table)) // If not a generic type
                }

            // TODO GENERIC: This should never actually happen, since it would be caught be the typechecker
            case t => assert(false, "Constructing primitive or bottom type"); null
        }
    ...
    private def generateFieldInitializer(
        ...
        def handleNonPrimitive(name: String, n: ObsidianType): Unit = {
            ...
            if (contract.isEmpty) {
                ...
                // using said GUID.
                val constructExpr = constructNew(translationContext, field.typ, inContract, model.ref("java.lang.String").dotclass())

                val elseBlock = checkForObj._else()
                val tryBlock = elseBlock._try()
                tryBlock.body().assign(fieldVar, constructExpr.arg(JExpr.ref(javaFieldName + "ID")))

                // We swallow all these exceptions related to reflection.
                // However, this is safe: the only strings we will use to fetch classes are strings that are also generated by us, so they will all be valid
                // Furthermore, all the classes are guaranteed to have the constructors we need, because we also generate those
                // TODO GENERIC: Verify this really is safe, or handle the exception better
                // TODO GENERIC: refactor this and the other place we use this code to be less repetitive
                tryBlock._catch(model.directClass(classOf[ClassNotFoundException].getCanonicalName))
                tryBlock._catch(model.directClass(classOf[InstantiationException].getCanonicalName))
                tryBlock._catch(model.directClass(classOf[NoSuchMethodException].getCanonicalName))
                tryBlock._catch(model.directClass(classOf[IllegalAccessException].getCanonicalName))
