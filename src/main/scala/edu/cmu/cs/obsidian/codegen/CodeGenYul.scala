...
object CodeGenYul extends CodeGenerator {
    ...
    /** given a symbol table, emit the collection of copying tracers for the contracts it defines,
      ...
      * @param ct the symbol table to process
      * @param name the current contract of interest
      * @return a sequence of Yul functions that trace the reachable contracts
      */
        ...
        for (d <- c.declarations) {
            d match {
                case Field(_, typ, fname, _) =>
                    ...
                    // inspect the type of the field to determine what to copy
                    typ match {
    ...
    def translateDeclaration(declaration: Declaration, contractName: String, checkedTable: SymbolTable): Seq[YulStatement] = {
        // todo there is a .asInstanceOf use in the mustache glue code that only works if this really
        ...
        e match {
            ...
            case e: BinaryExpression =>
                e match {
                    ...
                    case Divide(e1, e2) => call("sdiv", retvar, contractName, checkedTable, e1, e2)
                    case Multiply(e1, e2) => call("mul", retvar, contractName, checkedTable, e1, e2)
                    case Mod(e1, e2) => call("smod", retvar, contractName, checkedTable, e1, e2)
                    case Equals(e1, e2) => call("eq", retvar, contractName, checkedTable, e1, e2)
                    case GreaterThan(e1, e2) => call("sgt", retvar, contractName, checkedTable, e1, e2)
                    case GreaterThanOrEquals(e1, e2) => geq_leq("sgt", retvar, e1, e2, contractName, checkedTable)
                    case LessThan(e1, e2) => call("slt", retvar, contractName, checkedTable, e1, e2)
            ...
            case Construction(contractType, args, _, _) =>
