...
object CodeGenYul extends CodeGenerator {
    ...
    def defaultConstructorSignature(c : Contract, ct : SymbolTable): Seq[TypedName] = {
        def proc(d :Declaration): Seq[TypedName] = {
            d match {
                case Field(_, typ, name, _) => typ match {
                    case primitiveType: PrimitiveType => Seq(TypedName("con_arg_" + name, obsTypeToYulType(primitiveType)))
                    case npt: NonPrimitiveType => npt match {
                        case ContractReferenceType(contractType, _, _) =>
                            defaultConstructorSignature(ct.contractLookup(contractType.contractName).contract, ct)
                        case _ => Seq()
                    }
                    case _ => Seq()
                }
    ...
    def writeDefaultConstructor(c : Contract, ct: SymbolTable): FunctionDefinition = {
        addThisArgument(FunctionDefinition(name=flattenedName(c.name,fromMain = c.isMain, constructor = true),
            parameters = defaultConstructorSignature(c,ct),
            returnVariables = Seq(), //todo add a return for a this address?
            body = Block(Seq(LineComment("default constructor generated by the compiler")))))
