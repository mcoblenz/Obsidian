...
$x, y, z \in \textsc{Variables} 		\hfill 		\ell \in \textsc{IndirectRefs} 	\hfill 		o \in \textsc{ObjectRefs}$		\hfill		$f \in \textsc{FieldNames}$
\begin{grammar}
...
\and
\inferlbl{t-new}
	{ managed(C) \Rightarrow \Delta(\thisS) <: p' \permS manager(C)
	\\\\
	\overline{\tau\ \,f} = fields(C.S) }
	{ \Delta, \overline{x : \tau} \vdash_{b} \newS\ C.S(\overline{x})\ \asS\ p : p \permS C.S \dashv \Delta}
\and
\inferlbl{t-res-1}
	{ (\tau\ \,f) \in fields(\Delta(\thisS)) }
    	{ \Delta \vdash_{\falseS} \resS(f) : res(\tau) \dashv \Delta }
\and
\inferlbl{t-res-2}
	{ }
    	{ \Delta, x : \tau \vdash_{\trueS} \resS(x) : res(\tau) \dashv \Delta, x : \tau }
\and
\inferlbl{t-thr}
	{  }
...
\and
\inferlbl{t-inv-1}
	{ \big(TL\ \tau''\ m(\overline{\tau\ z}) \rightarrow \overline{S} \big) \in methods(\tau') \qquad p = \readonlyS \Rightarrow TL = \functionS }
	{ \Delta, x : p \permS \tau', \overline{y : \tau[z \mapsto y][\thisS \mapsto x]} \vdash_{\falseS} x.m(\overline{y}) : \tau'' \dashv \Delta, x : p \permS trans(\overline{S}, \tau') }
\and
\inferlbl{t-inv-2}
	{ (p \permS \tau'\ f) \in fields(\Delta(\thisS)) \qquad trans(\overline{S}, \tau') <: \tau'
	\\\\
	\big(TL\ \tau''\ m(\overline{\tau\ z}) \rightarrow \overline{S} \big) \in methods(\tau') \qquad p = \readonlyS \Rightarrow TL = \functionS }
	{ \Delta, \overline{y : \tau[z \mapsto y][\thisS \mapsto \thisS.f]} \vdash_{\falseS} f.m(\overline{y}) : \tau'' \dashv \Delta }
\and
\inferlbl{t-unpack}
	{ \Delta(\thisS) = p \permS C.S_{1} \qquad p \neq \readonlyS \qquad \overline{\tau\ f} =  mutableFields(C.S_{1}) 
	\\\\
	\forall i. f_{i} \not\in \Delta \qquad \Delta,\overline{ f : \tau}  \vdash_{\trueS} e : \packS[S_{2}, \tau] \dashv \Delta'}
	{ \Delta \vdash_{\falseS} \unpackS\ \{e\} : \tau \dashv rm(\overline{f}, \Delta') }
\and
\inferlbl{t-pack}
	{ \overline{\tau\ f} = mutableFields(trans(S, \Delta(\thisS))) }
...
\and
\inferlbl{t-assert}
	{ \Delta, x : p \permS \tau_{s}' \vdash_{t} e \dashv \Delta' }
	{ \Delta, x : p \permS \tau_{s} \vdash_{t} \assertS\ x : \tau_{s}'\ \inS\ e \dashv \Delta' }
\end{mathpar}
\end{minipage}
}
\caption{Statics}
...
\inferlbl{e-dyn-ex}
	...
	{ \mu, \rho, \assertS\ x : \tau_{s}\ \inS\ e \rightarrow \mu', \textsf{Ex} }
\and
\inferlbl{e-dyn-ok}
	{ type(\mu(\rho(x)))\ \textsf{instanceOf}\ \tau_{s} }
	{ \mu, \rho, \assertS\ x : \tau_{s}\ \inS\ e \rightarrow \mu', e }
\and
\inferlbl{e-try-1}
	{ }
