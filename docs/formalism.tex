...
$ \quad\qquad x, y, z, f \in \textsc{Variables} 		\hfill 		\ell \in \textsc{IndirectRefs} 	\hfill 		o \in \textsc{ObjectRefs} \qquad\quad $
\begin{grammar}
...
\boxed{\rmHelp(x, \Delta),\ \rmHelp(\overline{x}, \Delta)} 
\begin{mathpar}
\inferlbl{}
	{p \neq \ownedS}
...
\boxed{ \wfHelp(\Delta, C, \tau_{s}), \wfHelp(\Delta, C, \tau) }  Wellformedness of types w.r.t. $\Delta$ and $C$
\begin{mathpar}
\inferlbl{}
	{ \parentHelp(C) = D \qquad \tau_{b} = C \lor \tau_{b} = C.S }
	{ \wfHelp(\Delta, D, \tau_{b}) }
\and

\inferlbl{}
	{ \lnot \hasParentHelp(C) \qquad \tau_{b} = C \lor \tau_{b} = C.S }
	{ \wfHelp(\Delta, D, \tau_{b}) }
\and
\inferlbl{}
	{ \tau_{b} = C \lor \tau_{b} = C.S }
	{ \wfHelp(\Delta, C, \tau_{b}) }
\and
\inferlbl{}
	{ \Delta(x) = C' \qquad \wfHelp(\emptyset, C', P.\tau_{b}) \qquad \tau_{b} = C \lor \tau_{b} = C.S}
	{ \wfHelp(\Delta, D, x.P.\tau_{b}) }
\and
\inferlbl{}
	{ (\tau_{f}\ f) \in \constFieldsHelp(D) \qquad \wfHelp(\emptyset, \contractOfHelp(\tau_{f}), P.\tau_{b}) \qquad \tau_{b} = C \lor \tau_{b} = C.S}
	{ \wfHelp(\Delta, D, f.P.\tau_{b}) }
\end{mathpar}
\boxed{ \substHelp(\tau, x: \tau', M)} Substitution for Types
\begin{mathpar}
\inferlbl{}
	{ M(z) = y }
	{ \substHelp(p \permS z.\tau, x: \tau', M) = p \permS y.\tau }
\and
\inferlbl{}
	{ z \not\in \textsf{dom}(M) }
	{ \substHelp(p \permS z.\tau, x : \tau', M) = p \permS x.z.\tau }
\and
\inferlbl{}
	{ \lnot \hasParentHelp(C) \qquad \tau_{b} = C \lor \tau_{b} = C.S}
	{ \substHelp(p \permS \tau_{b}, x: \tau, M) = p \permS \tau_{b} }
\and
\inferlbl{}
	{ \hasParentHelp(C) \qquad \parentHelp(C) = \contractOfHelp(\tau) \qquad \tau_{b} = C \lor \tau_{b} = C.S }
	{ \substHelp(p \permS \tau_{b}, x: \tau, M) = p \permS x.\tau_{b}  }
\and
\inferlbl{}
	{ \contractOfHelp(\tau_{b}) = C }
	{ \substHelp(p \permS C, x: P.\tau_{b}, M) = p \permS P.C }
\and
\inferlbl{}
	{ \contractOfHelp(\tau_{b}) = C }
	{ \substHelp(p \permS C.S, x: P.\tau_{b}, M) = p \permS P.C.S }
\end{mathpar}
...
\inferlbl{t-var}
	{ }
    	{\Delta, x : \tau \vdash_{b} x : \tau \dashv \Delta, x : \resHelp(\tau) }
\and
\inferlbl{t-let}
	{ \forall P, \tau_{b}.\ \tau \neq x.P.\tau_{b} \qquad \forall \tau.\ (\tau\ x) \not\in \constFieldsHelp(\Delta(\thisS))
	\\\\
...
\inferlbl{t-read}
	{ (\tau\ \,f) \in \fieldsHelp(\Delta(\thisS)) }
    	{ \Delta \vdash_{b} \resS(f) : \resHelp(\tau) \dashv \Delta }
\and
\inferlbl{t-thr}
	{  }
...
\and
\inferlbl{t-inv}
	{ M = \emptyset\overline{[z \mapsto y]}
	\\\\
	\big(TL\ \tau_{ret}\ m(\overline{\tau\ z}) \rightarrow \overline{S} \big) \in \methodsHelp(\tau_{recip}) \qquad \tau_{ret}' = \substHelp(\tau_{ret}, x : \tau_{recip}, M)
	\\\\
	 p = \readonlyS \Rightarrow TL = \functionS \qquad \forall i.\ \tau_{i}' = \substHelp(\tau_{i}, x : \tau_{recip}, M) }
...
\and
\inferlbl{t-unpack}
	{  \overline{\tau\ f} =  \fieldsHelp(C.S_{1})
	\\\\
	p \neq \readonlyS \qquad \forall i. f_{i} \not\in \Delta \qquad \Delta,\overline{ f : \tau}  \vdash_{\trueS} e : \packS[S_{2}, \tau_{ret}] \dashv \Delta'}
	{ \Delta, \thisS : p \permS C.S_{1} \vdash_{\falseS} \unpackS\ \{e\} : \tau_{ret} \dashv \rmHelp(\overline{f}, \Delta'), \thisS: p \permS C.S_{2} }
\and
\inferlbl{t-pack}
	{ \overline{\tau_{c}\ f_{c}} = \constFieldsHelp(\Delta(\thisS)) \qquad  \overline{\tau_{m}\ f_{m}} = \mutableFieldsHelp(\transHelp(S, \Delta(\thisS)))
	\\\\
	\Delta(f_{c_{i}}) <: \tau_{c_{i}} \qquad \Delta' = \resHelp(\overline{f_{c}}, \Delta), y : \resHelp(\tau'), \overline{x : \resHelp(\tau_{m})} }
	{ \Delta, y : \tau', \overline{x : \tau_{m}} \vdash_{\trueS} \packS\ S(\overline{x})\ \withS\ y : \packS[S, \tau'] \dashv  \Delta'}
\and
\inferlbl{t-try}
