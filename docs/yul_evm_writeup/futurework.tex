...
The next step for benchmarking would be to do a side by side comparison
with reference Solidity examples, rather than just quantifying what happens
in the presence of garbage collection or not in our own environment. It
would be of particular interest to do a comparison to several live
implementation of linked lists that are actually deployed and used.
